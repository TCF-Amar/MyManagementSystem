...
$getAccessSqlQuery = "SELECT accesses FROM accesses WHERE emp_id = ?";
$stmt = $conn->prepare($getAccessSqlQuery);
    ...
    if (isset($_POST['registerNewStudent'])) {
        ...
        if (empty($rollNo) || empty($enrollNo) || empty($firstName) || empty($lastName) || empty($fatherName) || empty($contactNo) || empty($emailId) || empty($gender) || empty($dob) || empty($address) || empty($course) || empty($year)) {
            ...
            try {
                ...
                $sql = "INSERT INTO students (rollNo, enrollNo, firstName, lastName, fatherName, contact, email, gender, dob, address, courseId, courseYear) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssssssssssii", $rollNo, $enrollNo, $firstName, $lastName, $fatherName, $contactNo, $emailId, $gender, $dob, $address, $course, $year);
...
<body>
    ...
    <main class="main" id="main">
        <nav class="fixed top left-0 w-full bg-gray-600 font-bold text-white px-2" style="top:10vh; z-index: 9999">
            ...
            <button id="registerNewStudentBtn" type="button"
                class="hover:text-blue-600  <?php echo (in_array('Add Student', $AccessArray) ? '' : 'hidden') ?>">Register
                New Student</button>
        ...
        <div class="overflow-hidden overflow-x-auto mt-6">
            ...
            <table id="myTable" class="border-2 border-collapse table-fixed">
                <thead>
                    <tr class="border-2 border-black">
                        <th class="border-2 border-black overflow-hidden text-ellipsis hover:text-clip">Roll No</th>
                        <th class="border-2 border-black overflow-hidden text-ellipsis hover:text-clip">Enrollment No
                        </th>
                        <th class="border-2 border-black overflow-hidden text-ellipsis hover:text-clip">First Name</th>
                        <th class="border-2 border-black overflow-hidden text-ellipsis hover:text-clip">Last Name</th>
                        <th class="border-2 border-black overflow-hidden text-ellipsis hover:text-clip">Course</th>
                        <th class="border-2 border-black overflow-hidden text-ellipsis hover:text-clip">Year</th>
                        <?php if (in_array('Show Student Details', $AccessArray)): ?>
                            <th class="border-2 border-black overflow-hidden text-ellipsis hover:text-clip">Details</th>
                        <?php endif; ?>
                        <?php if (in_array("Update Student", $AccessArray) || in_array("Delete Student", $AccessArray)): ?>
                            <th class="border-2 border-black overflow-hidden text-ellipsis hover:text-clip">Action</th>
                        <?php endif; ?>
                    </tr>
                </thead>
                <tbody>
                    ...
                    while ($row = $result->fetch_assoc()): ?>
                        <tr class="border-2 border-black">
                            <td class="border border-black overflow-hidden text-ellipsis hover:text-clip">
                                <?php echo htmlspecialchars($row['rollNo']); ?>
                            </td>
                            <td class="border border-black overflow-hidden text-ellipsis hover:text-clip">
                                <?php echo htmlspecialchars($row['enrollNo']); ?>
                            </td>
                            <td class="border border-black overflow-hidden text-ellipsis hover:text-clip">
                            ...
                            <?php if (in_array('Show Student Details', $AccessArray)): ?>
                                <td class="border border-black overflow-hidden text-ellipsis hover:text-clip">
                                    <button class="w-full text-center text-blue-600 font-bold" id="showStudentDetailsBtn"
                                        data-studentId="<?php echo $row['id']; ?>">
                                        <i class="fa-solid fa-eye"></i><span>Show</span>
                                    </button>
                                </td>
                            <?php endif; ?>
                            <?php if (in_array("Update Student", $AccessArray) || in_array("Delete Student", $AccessArray)): ?>
                                <td class="border border-black overflow-hidden text-ellipsis font-bold hover:text-clip">
                                    <div class="w-full flex justify-center items-baseline gap-4">
                                        <?php if (in_array("Update Student", $AccessArray)): ?>
                                            <button class="w-full text-center text-blue-500 hover:text-blue-700"
                                                id="studentUpdateBtn" data-studentId="<?php echo $row['id']; ?>">
                                                <i class="fa fa-pen"></i>
                                                <span class='md:inline-block hidden'>Edit</span>
                                            </button>
                                        <?php endif; ?>
                                        <?php if (in_array("Delete Student", $AccessArray)): ?>
                                            <form method="post"
                                                onsubmit="return confirmDelete('<?php echo $row['firstName'] . ' ' . $row['lastName'] . ' (' . $row['courseName'] . ', ' . $row['courseYear'] . ')' ?>')">
                                                <input type="hidden" name="deleteStudent" value="<?php echo $row['id']; ?>">
                                                <button type="submit"
                                                    class='w-full text-center text-nowrap  text-red-500 hover:text-red-700'>
