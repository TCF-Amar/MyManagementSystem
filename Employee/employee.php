...
<body>
    ...
    <main class="main" id="main">
        <nav class="fixed top left-0 w-full bg-gray-600 font-bold text-white px-2" style="top:10vh; z-index: 9999">
            <button id="registerNewEmployeeBtn" onclick="openModal()" type="button"
                class="hover:text-blue-600 <?php echo (in_array('Add Employee', $AccessArray) ? '' : 'hidden') ?>">
                ...
                <table id="myTable" class="border-2 border-collapse table-fixed overflow-hidden overflow-x-auto">
                    <thead>
                        <tr class="border-2 border-black">
                            ...
                            <?php if (in_array('Show Employee Details', $AccessArray)): ?>
                                <th class="border-2 border-black overflow-hidden text-ellipsis">Details</th>
                            <?php endif; ?>

                            <?php if (in_array('Give Accesses Employee', $AccessArray)): ?>
                                <th class="border-2 border-black overflow-hidden text-ellipsis">Access</th>
                            <?php endif; ?>

                            <?php if (in_array('Update Employee', $AccessArray) || in_array('Delete Employee', $AccessArray)): ?>
                                <th class="border-2 border-black overflow-hidden text-ellipsis">Action</th>
                            <?php endif; ?>
                        </tr>
                    </thead>
                    <tbody>
        ...
        ORDER BY firstName ASC";
                        ...
                        if ($result && $result->num_rows > 0):
                            while ($row = $result->fetch_assoc()): ?>
                                <tr class='border-2 border-black'>
                                    <td class='border border-black overflow-hidden text-nowrap text-ellipsis hover:text-clip'>
                                    ...
                                    <?php if (in_array('Show Employee Details', $AccessArray)): ?>
                                        <td class='border border-black overflow-hidden text-center text-ellipsis'>
                                            <button data-empId='<?php echo $row['id'] ?>' id='employeeDetailsBtn'
                                    ...
                                    <?php if (in_array('Give Accesses Employee', $AccessArray)): ?>
                                        <td class='border border-black text-center overflow-hidden text-ellipsis'>
                                            <button id='empAccessBtn'
                                                class='text-blue-500 font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline'
                                                data-empId='<?php echo $row['id'] ?>'>
                                    ...
                                    <?php if (in_array('Update Employee', $AccessArray) || in_array('Delete Employee', $AccessArray)): ?>
                                        <td class='border  text-center border-black overflow-hidden text-ellipsis '>
                                            <?php if (in_array('Delete Employee', $AccessArray)): ?>
                                                <div>
                                                    <form action='' method='post'
                                                        onsubmit='return confirmEmpDelete("<?php echo $row['firstName'] . " " . $row['lastName'] ?>")'>
                                                        <input type='hidden' name='deleteEmp' value='<?php echo $row['id'] ?>'>
                                                        <button type='submit'
                                            ...
                                            <?php endif; ?>
                                            <?php if (in_array('Update Employee', $AccessArray)): ?>
                                                <div>
                                                    <button data-empId='<?php echo $row['id'] ?>'
                                                        class='text-blue-500 font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline'
                                                        id='empUpdateBtn'>
